...
class BaseModel(ABC):
    ...
    def mask_parameters(self, adjacencies):  # 该方法不执行任何有意义的操作，目的是作为一个占位符，表明该方法的实际实现预计在子类或开发后期阶段提供
        pass  # 不做任何处理
    ...
    @abstractmethod
    def sample(self, key, masked_dists, num_samples): # 从P_F中采样cpd参数
    ...
    @abstractmethod
    def sample_obs_and_log_probs(self, key, data, theta, parents):  # 从P(X_i | X_rest, \theta_i, Pa(X_i))中采样观测值并返回样本概率的对数
...
class GaussianModel(BaseModel):
    ...
    def log_likelihood(self, dataset, thetas, masks):
        @partial(jax.vmap, in_axes=(0, 1, 0), out_axes=1)  # 修饰器，将jax.vmap部分应用于函数_log_likelihood
        def _log_likelihood(theta, y, mask):
            mean = self.model(theta, dataset.data, mask)   # 这个mean是啥？
